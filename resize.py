...
def resize_images(input_folder, output_folder, width, height):
    ...
    for file_name in files:
        image = pdb.gimp_file_load(os.path.join(input_folder, file_name), file_name)
        ...
        transparent_layer = pdb.gimp_layer_new(new_image, 100, 100, RGBA_IMAGE, "Transparent Layer", 100, NORMAL_MODE)
        pdb.gimp_image_add_layer(new_image, transparent_layer, 0)
        
        margin_size = (100 - width) // 2
        
        new_layer = pdb.gimp_layer_new(new_image, width + 2 * margin_size, height + 2 * margin_size, RGBA_IMAGE, "Resized Layer", 100, NORMAL_MODE)
